...
```
但是！在运行的时候会发现存在一个问题：**这个函数的默认执行频率，太！高！了！**。 高到什么程度呢？以chrome为例，我们可以点击选中一个页面的滚动条，然后点击一次键盘的【向下方向键】，会发现函数执行了**8-9次！**
...
上述内容基于防抖和节流的核心思路设计了简单的实现算法，但是不代表实际的库（例如undercore js）的源码就直接是这样的，最起码的可以看出，在上述代码实现中，因为`showTop`本身的很简单，无需考虑作用域和参数传递，所以连`apply`都没有用到，实际上肯定还要考虑传递`argument`以及上下文环境（毕竟apply需要用到this对象）。